...
export default function BoxOfficeTbody({List,setSelMv}) {
    ...
    const tags = List.map(item =>  <tr key={item.movieCd} onClick={()=>{handleMvSelect(item)}} className="border-b border-neutral-200 hover:bg-neutral-100">
    ...
    <td className="px-6 py-3  flex justify-center items-center">
        ...
        <span>{parseInt(item.rankInten)!==0 && Math.abs(item.rankInten)}</span></td>
    </tr>)
